...
(function () {
  ...
  if (process.platform === 'darwin') {
    ...
    mb.createMacBuiltin("EASI Calculator");
...
$(document).ready(function () {
  ...
  $( "#version_display" ).html(VERSION);
  // Override JS alert with Vex alert
  window.alert = function(msg) {
    vex.dialog.alert(msg);
  }
  // Check for updates
  var request = require('request');
  request('http://www.easicalc.uk/updates-desktop.php?ver=' + VERSION, function (err, resp, body) {
    if (!err && resp.statusCode == 200) {
      // process update information
      $( '#updateInformation' ).html(body);
    }
  });
...
var easi;
var areaOneZero = false, areaTwoZero = false, areaThreeZero = false, areaFourZero = false;
    ...
    if (isNaN(easi) == true || allFieldsFilledIn() == false) {
      ...
      switch (true) {
        case (easi === 0):
          ...
          break;
        case (easi <= 1):
          band = "<span style='color:#0740A1'>Almost clear (0.1 to 1.0)</span>";
          break;
        case (easi <= 7):
          band = "<span style='color:#07B040'>Mild (1.1 to 7.0)</span>";
          break;
        case (easi <= 21):
          band = "<span style='color:#C9850B'>Moderate (7.1 to 21.0)</span>";
          break;
        case (easi <= 50):
          band = "<span style='color:#FF4817'>Severe (21.1 to 50.0)</span>";
          break;
        case (easi <= 72):
          band = "<span style='color:#BF1312'>Very severe (50.1 to 72.0)</span>";
          break;
        case (easi > 72):
          band = "Error. (Please send an email to bugs@easicalc.uk letting us know what numbers you inputted. Thanks!)";
          break;
      }
      alert("EASI = " + easi + "<br />" + band);
    }
  } else {
...
function checkInput() {
  ...
  // If applicable, grey out inputs
  function greyOutIfApplicable(els, val, form) {
    var x;
    if (parseFloat(val) == 0) {
      for (var el in els) {
        form.reset();
        form.area.value = "0";
        els[el].placeholder = "N/A";
        els[el].disabled = true;
      }
      x = true;
    } else {
      for (var el in els) {
        els[el].placeholder = "Score (0-3)";
  ...
  areaOneZero = greyOutIfApplicable([oneOneElement, oneTwoElement, oneThreeElement, oneFourElement], oneArea, document.regionOneForm);
  areaTwoZero = greyOutIfApplicable([twoOneElement, twoTwoElement, twoThreeElement, twoFourElement], twoArea, document.regionTwoForm);
  areaThreeZero = greyOutIfApplicable([threeOneElement, threeTwoElement, threeThreeElement, threeFourElement], threeArea, document.regionThreeForm);
  areaFourZero = greyOutIfApplicable([fourOneElement, fourTwoElement, fourThreeElement, fourFourElement], fourArea, document.regionFourForm);
...
function getInput() {
  ...
  if (areaOneZero) {
    oneOne = oneTwo = oneThree = oneFour = 0;
  }
  if (areaTwoZero) {
    twoOne = twoTwo = twoThree = twoFour = 0;
  }
  if (areaThreeZero) {
    threeOne = threeTwo = threeThree = threeFour = 0;
  }
  if (areaFourZero) {
    fourOne = fourTwo = fourThree = fourFour = 0;
  }
};
